...
// Creating a query that will return all todos that have the same uid as the current user.
...
// Getting all the todos from the database and then updating the complete status of all the todos that have the same uid as the current user.
...
// Getting all the todos from the database and then deleting all the todos that have the same uid as the current user and are complete.
...
// Updating the name of the todo 
  function changeName(event, todo) {
    console.log(
      todo.text + "(" + todo.id + ") name changed to " + event.target.value
    );
    db.collection("todos").doc(todo.id).update({ text: event.target.value });
  }

// these functions are used to drag and drop the todos
...
//   This function is used to deselect the input field when the user presses the enter key.
...
<!-- This is the form that the user uses to add a new todo. -->
<form
  class="form"
  id="newItemForm"
  autocomplete="off"
...
<div class="lists">
  <div class="list incomplete">
    <div class="headerBar2">
      ...
      <button class="button header2" on:click={updateAllIncomplete}
        >complete all</button
      >
    </div>
    ...
    {#each $todos.filter((t) => !t.complete) as todo (todo.id)}
      <div
        ...
        draggable={true}
        class:is-active={hovering === todo.id &&
          hoveringComplete === todo.complete}
      >
        <button class="check" on:click={updateStatus(todo)} />
        <span class="checkmark" />
        <input
          class="name"
          on:change={(event) => changeName(event, todo)}
  ...
  <div class="list complete">
    <div class="headerBar2">
      ...
      <button class="button header2" on:click={deleteAllComplete}
        >delete all</button
      >
    </div>

    <!-- shows complete todos -->
    {#each $todos.filter((t) => t.complete) as todo (todo.id)}
      <div
        class="line"
        in:receive={{ key: todo.id }}
      ...
      >
        <button class="check" on:click={updateStatus(todo)} />
        <span class="checkmark completed" />
